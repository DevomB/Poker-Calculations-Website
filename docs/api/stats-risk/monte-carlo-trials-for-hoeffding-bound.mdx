...
sidebar_label: monteCarloTrialsForHoeffdingBound
description: "Hoeffding: smallest `n` with `n ≥ ln(2/δ)/(2ε²)` for uniform MC error bound."
---

## Summary

Hoeffding: smallest `n` with `n ≥ ln(2/δ)/(2ε²)` for uniform MC error bound.
...
**Concrete trigger:** Hoeffding: smallest `n` with `n ≥ ln(2/δ)/(2ε²)` for uniform MC error bound.
...
// monteCarloTrialsForHoeffdingBound(...) — see signature above
```

Runnable sample in the repo: [NPM/examples/stats-risk.mjs](https://github.com/DevomB/Poker-Calculations/blob/main/NPM/examples/stats-risk.mjs).
