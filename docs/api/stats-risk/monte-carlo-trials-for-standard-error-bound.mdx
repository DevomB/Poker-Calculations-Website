...
sidebar_label: monteCarloTrialsForStandardErrorBound
description: "Smallest integer `n` with SE ≤ `targetSe` at interior `pHat` (ceil of `p(1−p)/se²`)."
---

## Summary

Smallest integer `n` with SE ≤ `targetSe` at interior `pHat` (ceil of `p(1−p)/se²`).
...
**Concrete trigger:** Smallest integer `n` with SE ≤ `targetSe` at interior `pHat` (ceil of `p(1−p)/se²`).
...
// monteCarloTrialsForStandardErrorBound(...) — see signature above
```
