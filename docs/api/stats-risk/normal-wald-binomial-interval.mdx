...
sidebar_label: normalWaldBinomialInterval
description: "Normal (Wald) `p̂ ± z·SE` clamped to `[0,1]` (weak near 0/1 with small `n`)."
---

## Summary

Normal (Wald) `p̂ ± z·SE` clamped to `[0,1]` (weak near 0/1 with small `n`).
...
**Concrete trigger:** Normal (Wald) `p̂ ± z·SE` clamped to `[0,1]` (weak near 0/1 with small `n`).
...
// normalWaldBinomialInterval(...) — see signature above
```
