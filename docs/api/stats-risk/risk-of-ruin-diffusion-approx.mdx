...
sidebar_label: riskOfRuinDiffusionApprox
description: "`exp(−2μB/σ²)` style ROR; returns `1` if drift ≤ 0."
---

## Summary

`exp(−2μB/σ²)` style ROR; returns `1` if drift ≤ 0.
...
**Concrete trigger:** `exp(−2μB/σ²)` style ROR; returns `1` if drift ≤ 0.
...
// riskOfRuinDiffusionApprox(...) — see signature above
```
