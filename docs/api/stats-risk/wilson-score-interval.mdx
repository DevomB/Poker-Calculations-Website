...
sidebar_label: wilsonScoreInterval
description: "Wilson interval; returns `{ lower, upper }`."
---

## Summary

Wilson interval; returns `{ lower, upper }`.
...
**Concrete trigger:** Wilson interval; returns `{ lower, upper }`.
...
// wilsonScoreInterval(...) — see signature above
```
