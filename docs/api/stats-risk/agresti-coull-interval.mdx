...
sidebar_label: agrestiCoullInterval
description: "Agresti–Coull interval (same `{ lower, upper }` shape)."
---

## Summary

Agresti–Coull interval (same `{ lower, upper }` shape).
...
**Concrete trigger:** Agresti–Coull interval (same `{ lower, upper }` shape).
...
// agrestiCoullInterval(...) — see signature above
```
