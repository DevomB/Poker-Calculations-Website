...
sidebar_label: estimatedOutsFromRuleOfTwo
description: "Uncapped inverse-style estimate `equity×unseen/2` clamped to `[0, unseen]` (not exact inverse of capped rule-of-two)."
---
...
**Concrete trigger:** Uncapped inverse-style estimate `equity×unseen/2` clamped to `[0, unseen]` (not exact inverse of capped rule-of-two).
...
// estimatedOutsFromRuleOfTwo(...) — see signature above
```
