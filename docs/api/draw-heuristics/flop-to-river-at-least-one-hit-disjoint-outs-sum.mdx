...
sidebar_label: flopToRiverAtLeastOneHitDisjointOutsSum
description: "Sum **disjoint** categories, then same two-street hit formula (categories must not share outs)."
---
...
**Concrete trigger:** Sum **disjoint** categories, then same two-street hit formula (categories must not share outs).
...
// flopToRiverAtLeastOneHitDisjointOutsSum(...) — see signature above
```
