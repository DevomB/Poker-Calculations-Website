...
sidebar_label: chubukovMaxSymmetricJamStackFromHandBinarySearch
description: "Same integer search; `maxStackChips` read as **int32** in the binding (pair with the other export for large caps)."
---

## Summary

Same integer search; `maxStackChips` read as **int32** in the binding (pair with the other export for large caps).
...
**Concrete trigger:** Same integer search; `maxStackChips` read as **int32** in the binding (pair with the other export for large caps).
...
// chubukovMaxSymmetricJamStackFromHandBinarySearch(...) — see signature above
```
