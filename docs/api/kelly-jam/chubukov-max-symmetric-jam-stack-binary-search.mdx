...
sidebar_label: chubukovMaxSymmetricJamStackBinarySearch
description: "Same search on equity from `exactHuEquityVsRandomHand` (board 3–5); `maxStackChips` clamped like native `double` → int cap."
---

## Summary

Same search on equity from `exactHuEquityVsRandomHand` (board 3–5); `maxStackChips` clamped like native `double` → int cap.
...
**Concrete trigger:** Same search on equity from `exactHuEquityVsRandomHand` (board 3–5); `maxStackChips` clamped like native `double` → int cap.
...
// chubukovMaxSymmetricJamStackBinarySearch(...) — see signature above
```

Runnable sample in the repo: [NPM/examples/kelly-chubukov-jam.mjs](https://github.com/DevomB/Poker-Calculations/blob/main/NPM/examples/kelly-chubukov-jam.mjs).
