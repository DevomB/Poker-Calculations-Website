...
sidebar_label: chubukovSymmetricJamBreakevenStack
description: "Toy symmetric jam: `S = equity·dead/(1−2·equity)` for `equity < 0.5`; `+∞` if `equity > 0.5`."
---

## Summary

Toy symmetric jam: `S = equity·dead/(1−2·equity)` for `equity < 0.5`; `+∞` if `equity > 0.5`.
...
**Concrete trigger:** Toy symmetric jam: `S = equity·dead/(1−2·equity)` for `equity < 0.5`; `+∞` if `equity > 0.5`.
...
// chubukovSymmetricJamBreakevenStack(...) — see signature above
```
