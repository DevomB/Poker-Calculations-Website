...
sidebar_label: handRankCategoryOrder
description: "Integer order `0..9` for `evaluateHandCategory` labels (`highCard` … `royalFlush`)."
---

## Summary

Integer order `0..9` for `evaluateHandCategory` labels (`highCard` … `royalFlush`).
...
**Concrete trigger:** Integer order `0..9` for `evaluateHandCategory` labels (`highCard` … `royalFlush`).
...
// handRankCategoryOrder(...) — see signature above
```
