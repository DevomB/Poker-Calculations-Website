...
sidebar_label: harringtonMEffectiveActiveAntes
description: "Same denominator with explicit per-seat antes for active players only."
---

## Summary

Same denominator with explicit per-seat antes for active players only.
...
**Concrete trigger:** Same denominator with explicit per-seat antes for active players only.
...
// harringtonMEffectiveActiveAntes(...) — see signature above
```
