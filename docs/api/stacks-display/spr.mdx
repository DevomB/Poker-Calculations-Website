---
title: spr
sidebar_label: spr
description: "spr"
---

## Summary

Native export `spr` from the `poker-calculations` addon.

## Signature

```ts
spr(potChips: number, effectiveStackChips: number): number;
```

## Implementation notes

```text
_See TypeScript types in the repository._
```

## When to use

Use for SPR, blinds-relative stack metrics, Harrington M/Q, orbit cost, NL minimum raise toy sizing, and human-readable pot-odds strings—anything that turns chips into ratios for HUDs or notes.

**Concrete trigger:** See the summary below.

**Skip this export** when you need full multi-street game trees, opponent-specific ranges, or browser-side execution without the native addon—this library is Node + N-API.

## Example

```js
const poker = require('poker-calculations');
// spr(...) — see signature above
```

Runnable sample in the repo: [NPM/examples/stacks-display.mjs](https://github.com/DevomB/Poker-Calculations/blob/main/NPM/examples/stacks-display.mjs).

## See also

- [API index](/docs/api)
- Type definitions: [`NPM/index.d.ts`](https://github.com/DevomB/Poker-Calculations/blob/main/NPM/index.d.ts)
