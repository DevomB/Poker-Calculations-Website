...
sidebar_label: nlMinimumRaiseToTotal
description: "NL toy minimum **total** wager after a raise: current + `max(last increment, BB)`."
---

## Summary

NL toy minimum **total** wager after a raise: current + `max(last increment, BB)`.
...
**Concrete trigger:** NL toy minimum **total** wager after a raise: current + `max(last increment, BB)`.
...
// nlMinimumRaiseToTotal(...) — see signature above
```
