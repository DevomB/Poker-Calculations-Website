...
sidebar_label: breakevenCallEquityFromPotOddsDisplayRatio
description: "`1/(1+R)` from `potOddsRatioDisplay` ratio `R`; `0` when `R` is `+∞`."
---

## Summary

`1/(1+R)` from `potOddsRatioDisplay` ratio `R`; `0` when `R` is `+∞`.
...
**Concrete trigger:** `1/(1+R)` from `potOddsRatioDisplay` ratio `R`; `0` when `R` is `+∞`.
...
// breakevenCallEquityFromPotOddsDisplayRatio(...) — see signature above
```
