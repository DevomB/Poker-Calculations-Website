...
sidebar_label: equityToWinningOddsAgainst
description: "Book-style `(1−p)/p`; `+∞` at `p=0`."
---

## Summary

Book-style `(1−p)/p`; `+∞` at `p=0`.
...
**Concrete trigger:** Book-style `(1−p)/p`; `+∞` at `p=0`.
...
// equityToWinningOddsAgainst(...) — see signature above
```
