---
title: rakeFromPot
sidebar_label: rakeFromPot
description: "Rake `min(fraction×pot, cap)` for rake-adjusted helpers."
---

## Summary

Rake `min(fraction×pot, cap)` for rake-adjusted helpers.
...
**Concrete trigger:** Rake `min(fraction×pot, cap)` for rake-adjusted helpers.
...
// rakeFromPot(...) — see signature above
```
