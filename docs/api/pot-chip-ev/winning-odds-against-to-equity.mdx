...
sidebar_label: winningOddsAgainstToEquity
description: "`1/(1+o)` for `o≥0`; `0` when `o=+∞`."
---

## Summary

`1/(1+o)` for `o≥0`; `0` when `o=+∞`.
...
**Concrete trigger:** `1/(1+o)` for `o≥0`; `0` when `o=+∞`.
...
// winningOddsAgainstToEquity(...) — see signature above
```
