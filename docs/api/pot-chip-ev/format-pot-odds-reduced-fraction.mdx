...
sidebar_label: formatPotOddsReducedFraction
description: "Reduced integer ratio string `pot : to_call` (e.g. `100`,`50`→`2:1`); `toCall=0`→`∞:1`."
---

## Summary

Reduced integer ratio string `pot : to_call` (e.g. `100`,`50`→`2:1`); `toCall=0`→`∞:1`.
...
**Concrete trigger:** Reduced integer ratio string `pot : to_call` (e.g. `100`,`50`→`2:1`); `toCall=0`→`∞:1`.
...
// formatPotOddsReducedFraction(...) — see signature above
```
