...
sidebar_label: geometricPotAfterMatchedPotFractions
description: "Pot after `nRounds` of matched pot-fraction HU betting: `pot0 × (1 + 2f)^n`."
---

## Summary

Pot after `nRounds` of matched pot-fraction HU betting: `pot0 × (1 + 2f)^n`.
...
**Concrete trigger:** Pot after `nRounds` of matched pot-fraction HU betting: `pot0 × (1 + 2f)^n`.
...
// geometricPotAfterMatchedPotFractions(...) — see signature above
```

Runnable sample in the repo: [NPM/examples/reverse-implied-geometry.mjs](https://github.com/DevomB/Poker-Calculations/blob/main/NPM/examples/reverse-implied-geometry.mjs).
