---
title: breakevenFoldEquitySemiBluff
sidebar_label: breakevenFoldEquitySemiBluff
description: "Two-outcome model; may exceed 1 if line is −EV even if villain always folds."
---

## Summary

Two-outcome model; may exceed 1 if line is −EV even if villain always folds.

## Signature

```ts
breakevenFoldEquitySemiBluff(
    potBeforeHeroBet: number,
    heroBetSize: number,
    equityWhenCalled: number,
    totalPotIfCalled: number
  ): number;
```

## Implementation notes

```text
_See TypeScript types in the repository._
```

## When to use

Use when you are sizing bluffs or semi-bluffs and need breakeven fold equity, optionally with rake or across two streets with independent fold assumptions.

**Concrete trigger:** Two-outcome model; may exceed 1 if line is −EV even if villain always folds.

**Skip this export** when you need full multi-street game trees, opponent-specific ranges, or browser-side execution without the native addon—this library is Node + N-API.

## Example

```js
const poker = require('poker-calculations');
// breakevenFoldEquitySemiBluff(...) — see signature above
```

Browse grouped samples under [`NPM/examples/`](https://github.com/DevomB/Poker-Calculations/tree/main/NPM/examples).

## See also

- [API index](/docs/api)
- Type definitions: [`NPM/index.d.ts`](https://github.com/DevomB/Poker-Calculations/blob/main/NPM/index.d.ts)
