...
sidebar_label: cardStringsHaveDuplicate
description: "`true` if two entries parse to the same card; throws if any string is invalid."
---

## Summary

`true` if two entries parse to the same card; throws if any string is invalid.
...
**Concrete trigger:** `true` if two entries parse to the same card; throws if any string is invalid.
...
// cardStringsHaveDuplicate(...) — see signature above
```
